...
| 2   | 长时间录制               | **可行**。1080p/30fps 连续录制 60 分钟：1397 个分片全部完整、0 写入错误，JS 堆恒定 ~4.5MB，全部进程 RSS 一小时只增长 34MB，音画偏移无漂移。必须对每个分片做写后回读校验（见正文） |
...
在离屏文档中连续录制 **1080p/30fps · 60 分钟**（`node spikes/scripts/run-media.mjs --minutes 60 --height 1080`）：

| 指标                       | 结果                                                     |
| -------------------------- | -------------------------------------------------------- |
| 时长                       | 3600 秒（ffprobe 3599.97 秒）                            |
| 视频轨                     | 681 个分片 / 896MB（合成画面，实际码率约 2Mbps）         |
| 转写音频轨                 | 716 个分片 / 7.1MB（Opus 24kbps 为上限，静音段码率更低） |
| 分片完整性                 | 数量、每片大小与录制时记录完全一致；写后回读校验 0 错误  |
| 单个分片写入 OPFS 最长     | 49ms（写入队列从未积压）                                 |
| JS 堆（离屏文档）          | 4.1 → 4.5MB，全程 4.1–4.9MB，不随时长增长                |
| 全部 Chrome 进程 RSS       | 1168 → 1202MB（一小时 +34MB）                            |
| 插件 renderer 进程         | ~112% 单核、RSS ~285MB（软件编码；其他进程 CPU ≈ 0）     |
| 播放 / 拖动                | 拖到 10% / 50% / 90% 落点精确，每次 ≤ 45ms               |
| 转封装为普通 MP4           | 896MB 耗时 5.1 秒                                        |
| 音画同步（开头/中段/结尾） | 中位 7 / 8 / 7ms，无漂移                                 |

**一次作废的 2 小时录制**：先跑的 2 小时 1080p 录制，在 6 分钟后因验证环境事故（执行环境会话中断、同时有格式化工具误扫了浏览器 profile 目录）导致之后所有分片在 OPFS 中都是 0 字节——**但 `createWritable → write → close` 没有抛出任何错误**，录制端完全感知不到，直到结束时写 `meta.json` 失败才暴露。内存、CPU 在整整 2 小时中同样平稳（JS 堆 3.4–4.9MB，RSS 1166 → 1317MB），但分片数据无效，因此改为 60 分钟重测，并给分片写入加了回读校验。
...
- **视频分片间隔不是严格 5 秒**：Chrome 的 MP4 封装只能在关键帧处切分片。默认关键帧间隔不固定，视频分片平均 ~6 秒、最长 11 秒；设置 `videoKeyFrameIntervalDuration: 1000` 后关键帧最长 1.06 秒，60 分钟平均 5.3 秒一片，但仍偶发一次 10 秒间隔（该 timeslice 没有输出，合并到下一片）。音频（WebM）始终严格 5 秒
...
- 视频 MediaRecorder 设置 `videoKeyFrameIntervalDuration: 1000`，让视频分片接近 5 秒
- **每个分片写完后回读文件大小校验**，不一致立即报警 / 停止录制并提示用户：OPFS 写入失败可能不抛错（上面作废的 2 小时录制）
- 两路录制：视频轨（H.264 + AAC/Opus）+ 转写音频轨（Opus 24kbps，实测 ~7MB/小时，上限 10.8MB/小时）
...
- 第 7 节「分片每 5 秒落盘，崩溃最多丢失 5 秒」改为「音频最多丢失 5 秒，视频通常 5 秒、偶发 10 秒」
- CPU 指标保留，但以实机测量为准
...
- 用 mediabunny（MPL-2.0，纯 JS）把 fMP4 **只转封装、不重新编码**为普通 MP4（moov 在尾部）：边读边写，内存恒定；1080p 5 分钟 75MB 耗时 0.33 秒、60 分钟 896MB 耗时 5.1 秒；转封装前后解码 0 错误、时长一致
...
| 1080p · 60 分钟        | 7ms / −30 ~ 42         | 8 / −27~43   | 7 / −28~43  |
...
- **25MB 基本不用切**：转写音频用 Opus 24kbps，实测 ~7MB/小时（上限 10.8MB/小时），2 小时以内都不超限
